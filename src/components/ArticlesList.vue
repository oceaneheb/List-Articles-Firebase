...
import ArticleDetails from './Article.vue'; //erreur
    ...
    methods: {
        onDataChange(items) {
            ...
            items.forEach((item) => {
                let key = item.key;
                let data = item.val();
                _articles.push({ //return enlevé
                    key: key,
                    title: data.title,
                    description: data.description, //erreur
        ...
        removeAllArticles() {
            ArticleDataService.deleteAll().then(() => this.refreshList()).catch((e) => console.log(e));
        }
    },
